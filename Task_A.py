...
def adjust_brightness(video_path, frame_sample_size=300):
    ...
    is_night = overall_avg_brightness < brightness_threshold

    return is_night

# Function to overlay the watermark on a frame with alternating watermarks
def overlay_watermarks(frame, watermark1, watermark2, position=(0, 0), frame_count=0, switch_interval=5):
    watermark = watermark1 if (frame_count // switch_interval) % 2 == 0 else watermark2
...
# Function to blur faces in a video and overlay another video with a border and alternating watermarks
def blur_faces_and_overlay(input_video_path, output_video_path, cascade_classifier_path,
                                                  overlay_video_path, endscreen_path, watermark_paths, adjust_brightness=False):
    ...
    # Define the size for the overlay video
    overlay_width = frame_width // 4
    overlay_height = frame_height // 4
    border_thickness = 5  # Thickness of the black border

    # Offset from the corner
    offset_x = 80  # Horizontal offset from the top-left corner
    offset_y = 80  # Vertical offset from the top-left corner
    ...
    while True:
        ...
        # Overlay alternating watermarks
        frame_main = overlay_watermarks(frame_main, watermark1, watermark2, frame_count=frame_count, switch_interval=switch_interval)
...
# Main function to process the videos with alternating watermarks
def process_videos(video_paths, cascade_classifier_path, overlay_video_path, endscreen_path, watermark_paths):
    for video_path in video_paths:
        ...
        blur_faces_and_overlay(video_path, output_video_path, cascade_classifier_path,
                                                      overlay_video_path, endscreen_path, watermark_paths,
                                                      adjust_brightness=brightness_adjust)
...
# Process the videos with alternating watermarks
process_videos(video_paths, cascade_classifier_path, overlay_video_path, endscreen_path, watermark_paths)